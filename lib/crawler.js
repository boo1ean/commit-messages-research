...
var getCommits = function(repo, cb) {
	if (hasCache(repo)) {
...
var fetchCommits = function(repo, cb) {
	...
	var fetch = function(error, response, body) {
		l('Fetched 100 more commits for', repo);
...
module.exports = function(repos, o) {
	...
	if (o.secrets && o.secrets.client_id && o.secrets.client_secret) {
		sign = '&client_id=' + o.secrets.client_id + '&client_secret=' + o.secrets.client_secret;
	}
	...
	each(repos, function(repo) {
		getCommits(repo, function(commits, repo) {
			if (o.map) {
				commits = commits.map(o.map);
			}

			var filter = o.filter || Boolean;

			o.handler(commits.filter(filter), repo);
		});
	});
};
